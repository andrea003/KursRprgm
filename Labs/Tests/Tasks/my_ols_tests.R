...
test_that("Assignment: my_ols()", {
  ...
  expect_equal(object = dim(my_ols_CO2$beta_hat), c(2,1), 
               info = "Fel: beta_hat har inte en korrekt storlek (dim())")
  expect_equal(object = as.vector(round(my_ols_CO2$beta_hat, 4)), c(19.5003, 0.0177), 
               info = "Fel: Beräkningen av beta_hat är felaktig.")
  expect_equal(object = dim(my_ols_CO2$sigma2_hat), c(1,1), 
               info = "Fel: sigma2_hat har inte en korrekt storlek (dim())")
  expect_equal(object = as.vector(round(my_ols_CO2$sigma2_hat, 4)), c(90.512), 
               info = "Fel: Beräkningen av sigma2_hat är felaktig.")
  expect_equal(object = dim(my_ols_CO2$e_hat), c(84,1), 
               info = "Fel: e_hat har inte en korrekt storlek (dim())")
  expect_equal(object = as.vector(round(my_ols_CO2$e_hat, 4))[1:2], c(-5.1847, 7.7969), 
               info = "Fel: Beräkningen av e_hat är felaktig.")

  data(attitude)
  expect_silent(object = my_ols_attitude <- my_ols(X=attitude[,c(2,5,3), drop=FALSE], y = attitude[,1]))
  expect_equal(object = dim(my_ols_attitude$beta_hat), c(4,1), 
               info = "Fel: beta_hat har inte en korrekt storlek (dim())")
  expect_equal(object = as.vector(round(my_ols_attitude$beta_hat, 4)), c(12.8231,0.7379,0.0890,-0.0581), 
               info = "Fel: Beräkningen av beta_hat är felaktig.")
  expect_equal(object = dim(my_ols_attitude$sigma2_hat), c(1,1), 
               info = "Fel: sigma2_hat har inte en korrekt storlek (dim())")
  expect_equal(object = as.vector(round(my_ols_attitude$sigma2_hat, 4)), c(51.8594), 
               info = "Fel: Beräkningen av sigma2_hat är felaktig.")
  expect_equal(object = dim(my_ols_attitude$e_hat), c(30,1), 
               info = "Fel: e_hat har inte en korrekt storlek (dim())")
  expect_equal(object = as.vector(round(my_ols_attitude$e_hat, 4))[1:2], c(-11.1419, 0.3067), 
               info = "Fel: Beräkningen av e_hat är felaktig.")
})
